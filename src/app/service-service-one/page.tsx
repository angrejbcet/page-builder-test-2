...
import { service_service_onePageData, service_service_onePageMetadata, service_service_onePageJsonLd } from "@/content/pages/service-service-one";

export const metadata: Metadata = service_service_onePageMetadata;

export default function ServiceServiceOnePage() {
  const { sections } = service_service_onePageData;
  return (
    <>
      {service_service_onePageJsonLd ? (
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(service_service_onePageJsonLd) }}
        />
      ) : null}
