    ...
    <div className={cn("relative w-full flex items-center", hClass)}>
      <div className="absolute inset-0" style={{ backgroundColor: "#1f2937" }}>
        {(slide.image?.startsWith("data:") || slide.image?.startsWith("http")) && (
          <Image src={slide.image} alt={slide.headline || ""} className="object-cover" fill unoptimized />
        )}
...
export const HeroSlider: React.FC<HeroSliderProps> = ({ slides = [], variant = "slider", height = "large", elementStyles }) => {
  ...
  return (
    <div className={cn("relative w-full", hClass)}>
      ...
      {/* Navigation Arrows */}
      <button
        className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/30 text-white d-flex items-center justify-center hover:bg-black/50 transition-colors z-10 hidden @min-[768px]:flex"
        onClick={scrollPrev}
      ...
      <button
        className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/30 text-white d-flex items-center justify-center hover:bg-black/50 transition-colors z-10 hidden @min-[768px]:flex"
        onClick={scrollNext}
